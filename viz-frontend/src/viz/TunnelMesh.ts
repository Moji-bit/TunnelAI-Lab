...
export function createTunnelMesh(): TunnelParts {
  ...
  const shellMaterial = new THREE.MeshStandardMaterial({
    ...
    clippingPlanes: [new THREE.Plane(new THREE.Vector3(0, -1, 0), 5.6)],
  });
  ...
  const barrierMaterial = new THREE.MeshStandardMaterial({ color: 0xc6cbd5, roughness: 0.85, metalness: 0.05 });
  const shoulderMaterial = new THREE.MeshStandardMaterial({ color: 0x28313f, roughness: 0.95, metalness: 0.03 });
  const gantryMaterial = new THREE.MeshStandardMaterial({ color: 0x697383, roughness: 0.55, metalness: 0.25 });
  const vmsPanelMaterial = new THREE.MeshStandardMaterial({ color: 0x141920, emissive: 0x1f4f15, emissiveIntensity: 0.35, roughness: 0.6, metalness: 0.2 });
  ...
  for (const tube of [1, 2]) {
    ...
    const leftShoulder = new THREE.Mesh(new THREE.PlaneGeometry(0.8, TUNNEL_LENGTH_METERS), shoulderMaterial);
    leftShoulder.rotation.x = -Math.PI / 2;
    leftShoulder.position.set(tubeCenterX - (LANE_WIDTH + 1.55), 0.015, TUNNEL_LENGTH_METERS / 2);
    equipmentGroup.add(leftShoulder);

    const rightShoulder = leftShoulder.clone();
    rightShoulder.position.x = tubeCenterX + (LANE_WIDTH + 1.55);
    equipmentGroup.add(rightShoulder);

    const centerBarrier = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.65, TUNNEL_LENGTH_METERS), barrierMaterial);
    centerBarrier.position.set(tubeCenterX, 0.35, TUNNEL_LENGTH_METERS / 2);
    equipmentGroup.add(centerBarrier);
    ...
    for (let z = 110; z < TUNNEL_LENGTH_METERS; z += 160) {
      ...
      const fanCore = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.28, 0.2, 10), new THREE.MeshStandardMaterial({ color: 0x2a313b }));
      fanCore.rotation.z = Math.PI / 2;
      fanCore.position.copy(fan.position);
      equipmentGroup.add(fanCore);
    }
    ...
    for (let z = 170; z < TUNNEL_LENGTH_METERS; z += 180) {
      const gantry = new THREE.Mesh(new THREE.BoxGeometry(2 * LANE_WIDTH + 2.8, 0.14, 0.14), gantryMaterial);
      gantry.position.set(tubeCenterX, 5.8, z);
      equipmentGroup.add(gantry);

      const hangerL = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 1.3, 6), gantryMaterial);
      hangerL.position.set(tubeCenterX - 1.4, 5.15, z);
      equipmentGroup.add(hangerL);
      const hangerR = hangerL.clone();
      hangerR.position.x = tubeCenterX + 1.4;
      equipmentGroup.add(hangerR);

      const vmsPanel = new THREE.Mesh(new THREE.BoxGeometry(2.3, 0.52, 0.12), vmsPanelMaterial);
      vmsPanel.position.set(tubeCenterX, 4.9, z);
      equipmentGroup.add(vmsPanel);
    }
  }
